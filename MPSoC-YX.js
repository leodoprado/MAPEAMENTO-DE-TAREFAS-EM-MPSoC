...
testData.TEST.forEach(TEST => {
    ...
    while (Application.grafo_tarefas.length > contSearch){
        ...
        while(linhaSource <= auxY && colunaSource <= auxX){
            if(matriz[linhaSource][colunaSource].length >= contIndex){
        ...
        console.log("linha/coluna"+linhaSource+colunaSource)
        linhaTarget = linhaSource
        colunaTarget = colunaSource

        while(linhaTarget < auxY && colunaTarget < auxX){
            if(matriz[linhaTarget][colunaTarget].length >= contIndex){
